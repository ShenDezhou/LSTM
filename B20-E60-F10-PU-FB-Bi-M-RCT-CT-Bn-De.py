...
#
#            B     0.9653    0.9460    0.9556     56882
#            M     0.7533    0.8627    0.8043     11479
#            E     0.9612    0.9494    0.9552     56882
#            S     0.9321    0.9354    0.9337     47490
#
#    micro avg     0.9387    0.9387    0.9387    172733
#    macro avg     0.9030    0.9234    0.9122    172733
# weighted avg     0.9407    0.9387    0.9394    172733
...
bigrams = []
with codecs.open('pku_dic/pku_bigram_t1.utf8', 'r', encoding='utf8') as f:
    lines = f.readlines()
    for line in lines:
        line = line.strip()
        if len(line) > 0:
...
if MODE==2:
    ...
    zhwiki_biemb = numpy.load("model/zhwiki_biembedding_t1.npy")
    ...
    maximumc = Maximum()([embeddedd, embeddede])

    sumbigram = concatenate([embeddeda, maximuma, maximumb, maximumc])
    ...
    # model.save("keras/pretrained-extradim-dropout-bilstm-bn-t1.h5")
    ...
    with codecs.open('model/extra/pku_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/extra/pku_train_crfstates.pkl', 'rb') as fy:
            with codecs.open('model/extra/pku_train_lstm_l1l2model.pkl', 'wb') as fm:
                bx = fx.read()
                by = fy.read()
                X = pickle.loads(bx)
                y = pickle.loads(by)
                ...
                # print(m)
                model_json = model.to_json()
                with open("keras/pretrained-extradim-dropout-bilstm-bn-t1-arch.json", "w") as json_file:
                    json_file.write(model_json)
                model.save_weights("keras/pretrained-extradim-dropout-bilstm-bn-t1-weights.h5")
                # model.save("keras/pretrained-extradim-dropout-bilstm-bn-t1.h5")
                print('FIN')
...
if MODE == 3:
    ...
    with codecs.open('plain/pku_test.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('baseline/pku_test_pretrained-extradim-wide-dropout-bilstm-bn-t1_states.txt', 'w', encoding='utf8') as fl:
            # with codecs.open('model/extra/pku_train_lstm_l1l2model.pkl', 'rb') as fm:
            #     bm = fm.read()
            #     model = pickle.loads(bm)
            json_file = open('keras/pretrained-extradim-dropout-bilstm-bn-t1-arch.json', 'r')
            loaded_model_json = json_file.read()
            json_file.close()
            model = model_from_json(loaded_model_json)
            model.load_weights("keras/pretrained-extradim-dropout-bilstm-bn-t1-weights.h5")

            # model = load_model("keras/pretrained-extradim-dropout-bilstm-bn-t1.h5")
            model.summary()
