...
#
#            B     0.9721    0.9494    0.9606     56882
#            M     0.8696    0.7518    0.8064     11479
#            E     0.9612    0.9343    0.9476     56882
#            S     0.8777    0.9603    0.9172     47490
#
#    micro avg     0.9343    0.9343    0.9343    172733
#    macro avg     0.9201    0.8990    0.9079    172733
# weighted avg     0.9357    0.9343    0.9341    172733
...
if MODE==2:
    ...
    sumbigram = concatenate([embeddeda, maximuma, maximumb, embeddedd, embeddede, tridropout])
    ...
    with codecs.open('model/initial/pku_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/initial/pku_train_crfstates.pkl', 'rb') as fy:
            with codecs.open('model/initial/pku_train_lstmmodel.pkl', 'wb') as fm:
                ...
                # print(m)
                model_json = model.to_json()
                with open("keras/pretrained-extradim-trigram-dropout-bilstm-bn-arch.json", "w") as json_file:
                    json_file.write(model_json)
                model.save_weights("keras/pretrained-trigram-extradim-dropout-bilstm-bn-weights.h5")
                # model.save("keras/pretrained-extradim-dropout-bilstm-bn.h5")
                print('FIN')
...
if MODE == 3:
    ...
    with codecs.open('plain/pku_test.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('baseline/pku_test_pretrained-extradim-trigram-dropout-bilstm-bn_states.txt', 'w', encoding='utf8') as fl:
