...
from keras.layers import Dense, Embedding, add, Add, SpatialDropout1D, LSTM, Input, Bidirectional, Flatten,CuDNNLSTM, Lambda, Dropout, BatchNormalization, Average, concatenate,Reshape, Concatenate
...
#            S     0.9087    0.9112    0.9099     47490
#
#    micro avg     0.9070    0.9070    0.9070    172733
#    macro avg     0.8614    0.8779    0.8689    172733
# weighted avg     0.9095    0.9070    0.9080    172733
...
if MODE==2:
    ...
    embeddeds = [Embedding(type_size, word_size, input_length=maxlen, mask_zero=False)(i) for i in denseinputs]
    averagea = Average()([embeddeda, embeddedb])
    averageb = Average()([embeddedc, embeddedb])
    sumbigram = Add()([embeddeda, averagea, averageb])
    sumtypes = Add()(embeddeds)

    concat = concatenate([sumbigram, sumtypes])
    ...
    with codecs.open('model/multiple/pku_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/multiple/pku_train_crfstates.pkl', 'rb') as fy:
            with codecs.open('model/multiple/pku_train_lstmmodel.pkl', 'wb') as fm:
                ...
                # )
                # print(m)
                model.save("keras/lstm-multiple.h5")
                print('FIN')

if MODE == 3:
    STATES = list("BMES")
    with codecs.open('plain/pku_test.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('baseline/pku_test_lstmmultiple_states.txt', 'w', encoding='utf8') as fl:
            # custom_objects = {'CRF': CRF,
            #                   'crf_loss': crf_loss,
            #                   'crf_accuracy': crf_accuracy}
            model = load_model("keras/lstm-multiple.h5")
            model.summary()
