...
#
#            B     0.0331    0.0685    0.0446     56882
#            M     0.0080    0.0382    0.0132     11479
#            E     0.0000    0.0000    0.0000     56882
#            S     0.0000    0.0000    0.0000     47490
#
#    micro avg     0.0251    0.0251    0.0251    172733
#    macro avg     0.0103    0.0267    0.0145    172733
# weighted avg     0.0114    0.0251    0.0156    172733
    ...
    with codecs.open('model/multiple/pku_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/multiple/pku_train_crfstates.pkl', 'rb') as fy:
            with codecs.open('model/multiple/pku_train_lstmmodel.pkl', 'wb') as fm:
                ...
                # )
                # print(m)
                model.save("keras/B20-E30-F8-PU-CT-Bn-De-CRF.h5")
                print('FIN')

if MODE == 3:
    STATES = list("BMES")
    with codecs.open('plain/pku_test.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('baseline/pku_test_B20-E30-F8-PU-CT-Bn-De-CRF_states.txt', 'w', encoding='utf8') as fl:
            custom_objects = {"CRF":CRF,"crf_loss":crf_loss,"crf_accuracy":crf_accuracy}
            model = load_model("keras/B20-E30-F8-PU-CT-Bn-De-CRF.h5", custom_objects=custom_objects)
            model.summary()
